...
export function sampleFilledAstraShape(svg, id) {
...
function buildFilledShape(svg, id) {
  const box = svg?.viewBox?.baseVal;
    ...
    try {
      ...
      return bounds.width > 0 && bounds.height > 0
        ? [{
            path,
            bounds,
            accent: path.hasAttribute("data-accent") &&
              path.getAttribute("data-accent") !== "false",
          }]
        : [];
    } catch {
      return [];
  ...
  function scan(rowCount) {
    ...
    for (const accent of [false, true]) {
      ...
      for (let row = 0; row < rowCount; row += 1) {
        ...
        const active = group.filter(({ bounds }) =>
        ...
        const contains = (x) => {
          point.x = x;
          point.y = y;
          return active.some(({ path, bounds }) =>
            x >= bounds.x && x <= bounds.x + bounds.width &&
            path.isPointInFill(point));
        };
        const boundary = (left, right, leftInside) => {
  ...
  for (let index = 0; index < routes.length; index += 1) {
    ...
    for (let sample = 0; sample < count; sample += 1) {
      ...
      const travel = sample <= turn
        ? sample / turn
        : (count - 1 - sample) / (count - 1 - turn);
