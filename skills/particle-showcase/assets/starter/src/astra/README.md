...
`custom-shapes.js` adds the exact whale paths from https://github.com/deepseek-ai/DeepSeek-V2/blob/main/figures/logo.svg. Four separate SVG subpaths preserve the body, mouth and eye details without connecting pen-up segments. `heroShape` opts into the opening path destination; original Astra behavior is preserved when omitted. The source project preserved its pre-customization baseline at `astra-original-v1`; its Git history is not part of this starter.

The final DeepSeek cue uses the custom titlecase `DeepSeek` lettering from `deepseek-titlecase.js`, with ten independent contours. Official lowercase e/e/p/e/e/k paths are translated, and genuine uppercase D/S outlines come from the bundled OpenAI Sans Medium font. This is a custom wordmark, not the official logotype. Only this wordmark increases fine-star visibility and scales point size, contour scatter and tracked flares to the viewport width. `uPathShapePointScale` defaults to 1, preserving the original shader result for Astra shapes and the whale.
