...
import { createAstraProfile } from "./profile";
import { sampleFilledAstraShape } from "../shapes/filled-sampler";

type DocumentBounds = ReturnType<typeof documentBounds>;
interface SceneAnchor {
  easing: AstraEasing;
  keyframe: AstraKeyframe;
  y: number;
}
interface MeasuredShape extends SampledElement {
...
interface SceneLayout {
  ...
  canvas: DocumentBounds;
  heroBounds: DocumentBounds;
  rangeBounds: DocumentBounds;
  cues: (AstraCueRegistration & { bounds: DocumentBounds })[];
  anchors: SceneAnchor[];
  runtime: PreparedRuntimeConfig[];
  shapes: MeasuredShape[];
  heroShape: SampledElement | null;
  intro: DocumentBounds | null;
  title: { bounds: DocumentBounds; padding: number; distance: number } | null;
}
type SceneEventMap = WindowEventMap &
  HTMLElementEventMap &
  DocumentEventMap & {
    ...
    "astra-replay": Event;
    "astra:replay": Event;
    "astra:rotate": CustomEvent<{ x: number; y: number }>;
    "astra:pointer": CustomEvent<{
      active: boolean;
      clientX: number;
      clientY: number;
      pressed?: boolean;
...
const smootherstep = (value: number, start: number, end: number) => {
...
export function sampleAstraPath(element: HTMLElement, id: string): SampledElement | null {
        ...
        ? [
            {
              path,
              length,
              matrix: rootMatrix && matrix ? rootMatrix.inverse().multiply(matrix) : null,
            },
          ]
        : [];
    } catch {
  ...
  for (let index = 0; index < 1024; index += 1) {
    ...
    while (pathIndex < paths.length - 1 && position > distance + paths[pathIndex].length) {
...
function getIntroElement(hero: Element, content: Element): Element | null {
  if (!content.contains(hero)) {
    return hero.compareDocumentPosition(content) & Node.DOCUMENT_POSITION_FOLLOWING
      ? (content.querySelector("[data-section-header]") ?? content.firstElementChild ?? content)
      : null;
  }
  let candidate: Element | null = hero;
  while (candidate && candidate !== content) {
    ...
    while (getComputedStyle(candidate).display === "contents" && candidate.firstElementChild) {
...
function cueRegistration(cue: HTMLElement | AstraCueRegistration): AstraCueRegistration {
  if ("element" in cue) return cue;
  let data: AstraCueData = {};
  try {
    const value = cue.dataset.astraScrollCue;
    if (value?.startsWith("{")) data = JSON.parse(value) as AstraCueData;
  } catch {
...
export function createAstraScene(
  canvas: HTMLCanvasElement,
  options: AstraSceneOptions = {},
): AstraSceneHandle {
  const resolvedHeroElement =
    options.heroElement ?? document.querySelector<HTMLElement>("[data-astra-hero]");
  const contentElement =
    options.contentElement ??
    document.querySelector<HTMLElement>("[data-astra-content]") ??
    document.body;
  if (!resolvedHeroElement) throw new Error("Astra requires a hero element.");
  const heroElement = resolvedHeroElement;
  const backdrop = canvas.closest<HTMLElement>("[data-astra-backdrop]") ?? canvas.parentElement;
  const experience =
    canvas.closest<HTMLElement>("[data-astra-experience]") ??
    contentElement.parentElement ??
    document.body;
  const chrome = [...document.querySelectorAll<HTMLElement>("[data-astra-copy]")];
  const dragSurfaces = [...document.querySelectorAll<HTMLElement>("[data-astra-drag]")];
  const title = document.querySelector<HTMLElement>("[data-astra-title]");
  const titleStage = title?.closest<HTMLElement>("[data-section-header]") ?? title?.parentElement;
  const heroData = options.data ?? {};
  const heroShapeElement = options.heroShape
    ? heroElement.querySelector<HTMLElement>("[data-astra-hero-shape]")
    : null;
  ...
  let active = true;
  let dragging: { id: number; target: HTMLElement; x: number; y: number } | null = null;
  let titleShift = 0;
  let resizeObserver: ResizeObserver | undefined;
  const removers: (() => void)[] = [];
  const previousTitlePadding = titleStage?.style.paddingBottom ?? "";
  const previousTitleTransform = title?.style.getPropertyValue("--astra-title-parallax-y") ?? "";
  const listen = <K extends keyof SceneEventMap>(
    target: EventTarget | null,
    event: K,
    callback: (event: SceneEventMap[K]) => void,
    listenerOptions?: AddEventListenerOptions,
  ) => {
    // DOM EventTarget erases event-name mappings; restore them at registration.
    const listener = callback as EventListener;
    target?.addEventListener(event, listener, listenerOptions);
    removers.push(() => target?.removeEventListener(event, listener, listenerOptions));
  };
  ...
  function measure() {
    ...
    const activation =
      heroBounds.top + Math.min(heroBounds.height, visual.height) * data.activationLine;
    ...
    let titleMetrics: SceneLayout["title"] = null;
    if (title && titleStage) {
      const trailingSpace = getAstraNumber(motion, "headingTrailingSpaceVh", 7, {
        min: 0,
        max: 100,
      });
      ...
      titleMetrics = {
        ...
        padding: Math.max(
          12,
          parseFloat(getComputedStyle(titleStage).getPropertyValue("--grid-gap")) || 0,
        ),
        distance: config.scrollEffects
          ? getAstraNumber(
              motion,
              mobile ? "headingParallaxDistanceMobile" : "headingParallaxDistance",
              mobile ? 144 : 240,
              { min: 0, max: 480 },
            )
          : 0,
    ...
    layout = {
      ...
      cues,
      anchors,
      runtime: anchors.map((anchor) => prepareAstraRuntimeConfig(config, anchor.keyframe)),
      shapes,
      heroShape,
    ...
    if (viewport.width !== width || viewport.height !== height || lastDpr !== dpr) {
  ...
  function resolveScroll() {
    ...
    let progress = 0;
    let sceneActive =
      heroBounds.top + heroBounds.height > scrollTop && heroBounds.top < scrollTop + visible.height;
    ...
    if (anchors.length > 1 && activation > anchors[0].y) {
      ...
      for (let index = 0; index < last; index += 1) {
        ...
        toIndex = index + 1;
        const fraction =
          (activation - anchors[index].y) / Math.max(1, anchors[index + 1].y - anchors[index].y);
    ...
    input.contentBounds = layout.title
      ? {
          left: Math.max(
            0,
            (layout.title.bounds.left - layout.title.padding - scrollX - layout.canvas.left) /
              layout.canvas.width,
          ),
          right: Math.min(
            1,
    ...
    if (config.scrollEffects && !input.reducedMotion) {
      const maxTop = Math.max(
        0,
        rangeBounds.top + rangeBounds.height - visible.height - visible.scrollOffsetTop,
      );
      ...
      let selected: MeasuredShape | undefined;
      for (const candidate of layout.shapes) {
        ...
        const release = candidate.holdAtRangeEnd ? 1 : 1 - smootherstep(fraction, 0.5, 0.86);
      ...
      if (selected) {
        ...
        shape.centerNdc.x =
          ((bounds.left - scrollX - layout.canvas.left + bounds.width / 2) / layout.canvas.width) *
            2 -
          1;
        shape.centerNdc.y =
          1 - ((bounds.top - clippedTop + bounds.height / 2) / layout.canvas.height) * 2;
    ...
    const scrollState = getAstraScrollState(heroScroll, config, input.reducedMotion);
    ...
    if (layout.intro && config.scrollEffects && !input.reducedMotion) {
      ...
      const scatterEnd = layout.intro.top + layout.intro.height / 2 - visible.height / 2;
      input.scatterProgress = clamp((scrollTop - scatterStart) / (scatterEnd - scatterStart));
      input.tiltProgress = clamp(heroScroll / Math.max(1, scatterEnd - heroBounds.top));
    }
    if (layout.heroShape) {
      ...
      if (strength >= shape.strength || shape.strength === 0) {
        ...
        const widthRatio = layout.heroShape.filled ? (aspectRatio > 2 ? 0.66 : 0.42) : 0.58;
    ...
    if (layout.title) {
      ...
      titleShift = input.reducedMotion
        ...
        : Math.round(
            clamp(
              (bounds.top + bounds.height / 2 - (scrollTop + visible.height / 2)) /
    ...
    const translateY = Math.min(
      0,
      Math.round(rangeBounds.top + rangeBounds.height - (scrollTop + layout.canvas.height)),
    );
    const withinRange = translateY > -layout.canvas.height;
    active = (sceneActive || shape.strength > 0.001) && withinRange && opacity > 0.01;
    input.starsOpacity = 1;
    const lastShape = layout.cues.filter((cue) => resolveAstraPathShape(cue.data?.shape)).at(-1);
    if (lastShape && resolveAstraPathShape(lastShape.data?.shape) === "openai-knot") {
      const bounds = lastShape.bounds;
      const knot = ASTRA_SHAPE_SVGS["openai-knot"];
      const height = Math.min(bounds.height, (bounds.width * knot.height) / knot.width);
      const bottom = bounds.top + (bounds.height + height) / 2;
      input.starsOpacity =
        1 -
        applyAstraEasing((scrollTop - bottom) / Math.max(1, 0.05 * visible.height), "smoothstep");
    }
    if (!active) clearPointer(true);
    if (backdrop) {
      backdrop.style.opacity = String(Math.round((active ? opacity : 0) * 10000) / 10000);
      backdrop.style.visibility = active ? "visible" : "hidden";
      backdrop.style.transform = translateY === 0 ? "" : `translate3d(0, ${translateY}px, 0)`;
      backdrop.style.setProperty("--astra-fallback-opacity", String(scrollState.textOpacity));
      if (active) backdrop.dataset.astraActive = "true";
      else delete backdrop.dataset.astraActive;
    }
    for (const element of chrome) {
      const opacity =
        (sceneActive || shape.strength > 0.001) && withinRange ? scrollState.textOpacity : 0;
  ...
  function render(delta: number) {
    ...
    const animationDelta = input.reducedMotion || !runtimeConfig.animationPlaying ? 0 : delta;
    ...
    renderer.render(animationDelta, animation, runtimeConfig, input.reducedMotion);
    return moving;
  }
  ...
  function tick(timestamp: number) {
    ...
    const delta = lastFrame === null ? 0 : clamp((timestamp - lastFrame) / 1000, 0, 0.05);
    lastFrame = timestamp;
    try {
      const moving = render(delta);
      if (
        (!failed && active && !input.reducedMotion && runtimeConfig.animationPlaying) ||
        moving ||
        input.scrolling
      )
        invalidate();
  ...
  function invalidate() {
    if (!disposed && initialized && frameId === null && document.visibilityState !== "hidden")
  ...
  function point(clientX: number, clientY: number, pressed: boolean) {
    if (input.reducedMotion || !Number.isFinite(clientX) || !Number.isFinite(clientY)) return;
  ...
  function release(event?: PointerEvent) {
    ...
    if (event?.pointerType === "mouse") point(event.clientX, event.clientY, false);
    else clearPointer();
  ...
  function replay() {
    ...
    lastFrame = null;
    const ambient = backdrop?.querySelector<HTMLElement>("[data-astra-ambient]");
    if (ambient) {
    ...
    chrome
      .flatMap((element) => [
        ...element.querySelectorAll<HTMLElement>("[data-astra-label-letter],.astra-label-letter"),
      ])
      .forEach((letter) => {
  ...
  listen(window, "astra:replay", replay);
  listen(window, "astra:rotate", (event) => event.detail && rotate(event.detail.x, event.detail.y));
  listen(window, "astra:pointer", (event) => {
    const pointer = event.detail;
    if (pointer?.active) point(pointer.clientX, pointer.clientY, pointer.pressed === true);
    else {
  ...
  for (const dragSurface of dragSurfaces) {
    listen(dragSurface, "pointerdown", (event) => {
      if (!event.isPrimary || event.button !== 0 || dragging || input.reducedMotion) return;
      try {
    ...
    listen(dragSurface, "pointermove", (event) => {
      ...
      point(event.clientX, event.clientY, true);
      rotate((event.clientX - dragging.x) * 0.005, (event.clientY - dragging.y) * 0.005);
  ...
  const ready = (async () => {
    ...
    if (disposed) return;
    if (failed) throw new Error("Astra lost its renderer during initialization.");
    measure();
    resolveScroll();
    animation = createAstraAnimationState(config, input.progress);
    initialized = true;
  ...
  return {
    ...
    dispose() {
      ...
      if (title) title?.style.setProperty("--astra-title-parallax-y", previousTitleTransform);
    },
  };
}
