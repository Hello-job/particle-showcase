...
模板基准为 React 19.2、Three.js 0.180.0、postprocessing 6.39.4、Vite 6.4.2。使用 `npm ci` 验证，不在初次复现时顺手升级。
...
| 新 Logo 不出现 | 注册表及 resolver 都有新 key；DOM cue 和配置一致；新 DOM 挂载后重建场景 |
