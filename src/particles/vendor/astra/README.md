...
The original shader source, point generation, seeded randomness, spline geometry, palette, pointer physics, optical postprocessing and animation math are retained in the extracted modules. `adapter.js` is a small module adapter that replaces the original site's bundler and supplies the installed `three` and `postprocessing` packages. No requests to the original website are made at runtime. Existing custom Logo extensions in `field.js` and `animation.js` are retained as well; this directory is a port, not an untouched upstream release.

## Why this directory is JavaScript

These files are extracted compiled modules, including large shader strings and mathematical code. They are deliberately isolated here instead of being renamed to `.ts` or hidden behind `@ts-nocheck`. The TypeScript migration preserves the nine renderer, geometry, motion, defaults and configuration modules byte for byte. Application code must use `../../engine/index.ts`, not this loader directly.

`adapter.d.ts` describes the functions consumed by the handwritten DOM coordinator. Vendor-created config, field and animation internals are opaque; the declaration exposes only the state required by that integration. This is the only untyped implementation boundary. The DOM coordinator, device profile, geometry registry and filled-path sampler are strict TypeScript outside this directory.
...
| `defaults.js`, `../../shapes/astra.ts` | `28nbglk5oli9p.js` | 170825, 59820, 651217, 333074 |
| `configuration.js`, DOM coordination reference | `0xnf_9ewj1sr3.js` | 682025, 330592, 777124, 9259 |
| `../../engine/profile.ts` | `23fu5ny4982ec.js` | 215453 |
...
Import `createAstraScene` and its options/handle types from `src/particles/engine/index.ts` for the DOM lifecycle. Add custom geometry to `src/particles/shapes/registry.ts`. Its `shapes.js` compatibility factory supplies that same typed registry to the original module loader, so the React layer and renderer always resolve the same IDs.
...
`../../shapes/deepseek.ts` adds the exact whale paths from https://github.com/deepseek-ai/DeepSeek-V2/blob/main/figures/logo.svg. Four separate SVG subpaths preserve the body, mouth and eye details without connecting pen-up segments. `heroShape` opts into the opening path destination; original Astra behavior is preserved when omitted. The pre-customization source is tagged `astra-original-v1`.

The original lowercase DeepSeek wordmark is preserved in `../../shapes/deepseek.ts`. The visible `DeepSeek` casing comes from `../../shapes/deepseek-titlecase.ts`: official lowercase contours plus uppercase D/S from the bundled font. It is a custom wordmark, not the official logotype. Only this wordmark increases fine-star visibility and scales point size, contour scatter and tracked flares to the viewport width. `uPathShapePointScale` defaults to 1, preserving the original shader result for all Astra shapes and the whale.

## Source rights

Public availability does not imply an open-source license. The extracted renderer, bundled fonts and brand assets have separate provenance and are not uniformly MIT licensed. Preserve this attribution and the repository's third-party notices when redistributing the demo or Skill.
